...
public:
  ...
  // Description:
  // Typically when you augment/remove to/from a stroke, only the incremental 
  // contributions are stored. ie you drew over a region that's already
  // been drawn earlier, the duplications are not stored into the sttroke.
  // This saves memory. However for certain applications where you might be
  // performing slice by slice edit and you want whatever you stroke to be
  // copied into a buffer etc, you may want the full non-incremental
  // data. See StoreNonIncrementalData. This is off by default. Turning it
  // ON will slightly hit performance (speed) and will slightly increase
  // memory consumption since an additional buffer depicting the non-
  // incremental contribution of the stroke will need to be stored.
  vtkGetObjectMacro( EtchData, vtkKWEPaintbrushData );
  static void SetStoreNonIncrementalData( int );
  static int GetStoreNonIncrementalData();
...
protected:
  ...
  // Description:
  // Allocate the non-incremental store if necessary. This is automatically
  // called by Allocate().
  virtual void AllocateEtchData();
