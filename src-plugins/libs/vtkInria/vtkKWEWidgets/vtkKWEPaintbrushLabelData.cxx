...
vtkCxxRevisionMacro(vtkKWEPaintbrushLabelData, "$Revision: 3596 $");
...
//----------------------------------------------------------------------------
// Replaces voxels with label of "from" to the value "to".
void vtkKWEPaintbrushLabelData::ReplaceLabel(
    vtkKWEPaintbrushEnums::LabelType from,
    vtkKWEPaintbrushEnums::LabelType to )
{
  if (from == to)
    {
    return;
  ...
  if (this->LabelMap->GetPointData()->GetScalars())
    {
    vtkDataArray * array = this->LabelMap->GetPointData()->GetScalars();
    vtkKWEPaintbrushEnums::LabelType *arrayPointer =
      static_cast<vtkKWEPaintbrushEnums::LabelType *>(array->GetVoidPointer(0));
...
{
  // Force computation of scalar range.
  if (this->LabelMap && 
      this->LabelMap->GetPointData() && 
      this->LabelMap->GetPointData()->GetScalars())
    {
    this->LabelMap->GetPointData()->GetScalars()->Modified();
    }
