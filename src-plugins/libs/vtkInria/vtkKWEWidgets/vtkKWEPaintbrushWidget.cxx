...
vtkCxxRevisionMacro(vtkKWEPaintbrushWidget, "1.14");
...
{
  vtkKWEPaintbrushTimer timer1("MoveCallback");
...
int vtkKWEPaintbrushWidget::BeginDrawAction(vtkKWEPaintbrushWidget *dispatcher)
{
  ...
  this->WaitCount = 0; // Reset the count.
  return 1;
}

//----------------------------------------------------------------------
int vtkKWEPaintbrushWidget::
EndStrokeAction( vtkKWEPaintbrushWidget *dispatcher )
{
  if (this->WidgetState == vtkKWEPaintbrushWidget::PaintbrushDisabled)
    {
    return 0;
  ...
  // If we are doing an interactive render, (ie avoiding render of the
  // paintbrush overlays on all except the active view), then render the other
  // views post interaction.
  if (this->WidgetGroup &&
      !this->WidgetGroup->GetInteractiveRenderAllWidgets() &&
      dispatcher != this)
    {
    this->Render();
    }
...
int vtkKWEPaintbrushWidget::HoverAction(vtkKWEPaintbrushWidget *dispatcher)
{
  ...
  vtkKWEPaintbrushTimer timer1("CreateShapeOutline");
  ...
  if (this == dispatcher &&
      (rep->GetInteractionState() == PaintbrushDraw ||
       rep->GetInteractionState() == PaintbrushErase))
    {
    vtkKWEPaintbrushTimer timer2("AddShapeToCurrentStroke");

    int changed = rep->AddShapeToCurrentStroke( d );

    // Every 5 interactions is a forced render. Else check if addition of a
    // shape at this location makes a difference and only then render.
    if (this->WaitCount % 5 && changed == 0)
      {
      return 0;
      }
    }
  ...
  // If we are in draw/erase, and if the InteractiveRenderAllWidgets
  // flag is OFF, render only the view that's being interacted with
  if (!this->WidgetGroup ||
      this->WidgetGroup->GetInteractiveRenderAllWidgets() ||
      this->WaitCount % 5 == 0 ||
      this == dispatcher ||
        !(rep->GetInteractionState() == PaintbrushDraw ||
          rep->GetInteractionState() == PaintbrushErase))
    {
    vtkKWEPaintbrushTimer timer3("Render widget");
    this->Render();
    }
...
{
  int retval = this->ToggleSelectStateActionInternal(dispatcher);
...
//-------------------------------------------------------------------------
int vtkKWEPaintbrushWidget
::ToggleSelectStateActionInternal(vtkKWEPaintbrushWidget *dispatcher)
{
  vtkKWEPaintbrushRepresentation *rep =
    vtkKWEPaintbrushRepresentation::SafeDownCast(this->WidgetRep);
...
void vtkKWEPaintbrushWidget::SetWidgetStateToEnabled()
{
  if (this->WidgetState == vtkKWEPaintbrushWidget::PaintbrushDisabled)
    {
    this->WidgetGroup->DispatchAction(this,
      &vtkKWEPaintbrushWidget::ToggleSelectStateActionInternal);      
    }
}
