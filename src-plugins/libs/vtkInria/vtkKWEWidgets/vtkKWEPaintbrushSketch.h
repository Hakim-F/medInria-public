...
public:
  ...
  virtual void SetLabel( vtkKWEPaintbrushEnums::LabelType );
  ...
  // sketch.
  void CopySketchFromExtentToExtent(int sourceExtent[6],
                                    int targetExtent[6]);

  // Description:
  // This method can be used to copy the last stroke of a sketch from a
  // source extent to a target extent. Only voxels that belong to the last
  // stroke of this sketch will be copied over. Use cases for this method
  // are slice by slice propagation of a sketch.
  void CopyLastStrokeFromExtentToExtent(int sourceExtent[6],
                                        int targetExtent[6]);
...
protected:
  ...
  // A return value of 0 indicates if the add resulted in no change to the
  // sketch. This is used upstream for speedups (avoid re-renders etc).
  virtual int AddShapeToCurrentStroke(
       double p[3],
