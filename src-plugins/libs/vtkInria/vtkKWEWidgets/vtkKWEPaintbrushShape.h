...
//ETX
  ...
  int GetPaintbrushData(vtkKWEPaintbrushData *, int x, int y, int z);
  int GetPaintbrushData(vtkKWEPaintbrushData *, double p[3]);
  ...
  // Description:
  // The shape will not be redrawn unless its displaced by more than
  // the tolerance distance away from its previous position. Too low a value
  // may result in the interface being slow. Too large a value may not
  // provide sufficient resolution for a draw.
  double GetPositionTolerance2();

protected:
  vtkKWEPaintbrushShape();
  ~vtkKWEPaintbrushShape();

  // Cache shape. This allows us to check if the shape data has changed since
  // a translation of the brush, thereby allowing us to avoid a re-addition
  // to the stroke, re-render. The method returns 1 if the cache was updated.
  int UpdateCache(vtkKWEPaintbrushData *);
  ...
  vtkKWEPaintbrushData * CachedShapeData;
